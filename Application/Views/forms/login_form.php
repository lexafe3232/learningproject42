...
<form method="post" name="login_form" id="login_form">
    ...
    <input name="email" id="email" type="email" value="<?=$_POST['email']; ?>" />
        ...
        <label for="email" class="form-error<? if ($this->isActiveError('auth_invalid_email')): ?> is-visible<? endif; ?>"><?=$this->getStr('error_auth_invalid_email'); ?></label>
        <label for="email" class="form-error<? if ($this->isActiveError('auth_user_not_found')): ?> is-visible<? endif; ?>"><?=$this->getStr('error_auth_user_not_found'); ?></label>
